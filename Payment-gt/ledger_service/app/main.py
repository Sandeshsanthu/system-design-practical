...
async def record_authorization_event(
    ...
    try:
        ...
        logger.exception(f"Failed to record authorization: {e}")
...
async def record_capture_event(
    ...
    try:
        ...
        logger.exception(f"Failed to record capture: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def record_refund_event(
    ...
    try:
        ...
        logger.exception(f"Failed to record refund: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def record_void_event(
    ...
    try:
        ...
        logger.exception(f"Failed to record void: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_account(account_id: str, db: Annotated[Session, Depends(get_db)]):
...
async def get_payment_entries(payment_id: str, db: Annotated[Session, Depends(get_db)]):
    entries = (
...
async def reconcile_payment(payment_id: str, db: Annotated[Session, Depends(get_db)]):
    entries = (
        db.query(LedgerEntry)
        .filter(LedgerEntry.payment_id == payment_id)
        .all()
    ...
    return {
        ...
        "events_recorded": list({e.event_type for e in entries}),
    }
