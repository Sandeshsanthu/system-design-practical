...
MERCHANT = {("POST","payment_intents"):"payment_intents:create",("POST","captures"):"captures:create",("POST","refunds"):"refunds:create",("GET","payments"):"payments:read",("GET","balance"):"balance:read"}
CUSTOMER = {("POST","payment_sessions"):"payment_session:create",("POST","payment_methods/tokenize"):"payment_method:tokenize",("POST","payment_intents/confirm"):"payment_intent:confirm_own",("GET","config"):"public_config:read"}
...
async def lifespan(app: FastAPI):
    ...
    yield
    await app.state.http.aclose(); await app.state.redis.aclose(); await app.state.db.close()
...
async def cors(request: Request, call_next):
    ...
    if origin and origin in settings.allowed_origins:
        r.headers["Access-Control-Allow-Origin"] = origin; r.headers["Access-Control-Allow-Credentials"] = "true"; r.headers["Vary"] = "Origin"
    return r
...
async def proxy(req: Request, base: str, path: str, extra: dict):
    ...
    try:
        up = await app.state.http.request(req.method, f"{base.rstrip('/')}/{path.lstrip('/')}", params=req.query_params, content=body, headers=headers)
...
async def merchant(path: str, request: Request):
    ...
    if scope: require_scope(record, scope)
    ...
    return await proxy(request, settings.merchant_service_url, path, {
        "X-Merchant-Id": record["merchant_id"], "X-Key-Id": record["key_id"], "X-Key-Type": record["key_type"]
    })
...
async def customer(path: str, request: Request):
    ...
    if not pk: raise HTTPException(status_code=401, detail="Missing publishable key")
    ...
    return await proxy(request, settings.customer_service_url, path, {
        "X-Merchant-Id": record["merchant_id"], "X-Key-Id": record["key_id"], "X-Key-Type": record["key_type"],
        "X-User-Id": str(claims.get("sub","")), "X-User-Email": str(claims.get("email","")),
