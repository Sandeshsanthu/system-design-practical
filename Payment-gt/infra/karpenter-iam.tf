  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Condition = {
          StringEquals = {
            # FIXED: was var.cluster_name — must match the full cluster name
            "aws:RequestTag/kubernetes.io/cluster/${local.full_cluster_name}" = "owned"
          }
          StringLike = {
      ...
      },
      {
        ...
        Condition = {
          StringEquals = {
            # FIXED: was var.cluster_name
            "aws:ResourceTag/kubernetes.io/cluster/${local.full_cluster_name}" = "owned"
          }
          StringLike = {
      ...
      },
      {
        ...
        Action = [
          ...
          "ec2:DescribeSubnets", "pricing:GetProducts", "ssm:GetParameter"
        ]
        Resource = ["*"]
      },
      {
      ...
      {
        Sid    = "AllowUnconditionedDiscovery"
        Effect = "Allow"
        Action = ["iam:GetInstanceProfile", "eks:DescribeCluster", "ec2:CreateTags"]
        Resource = ["*"]
      }
    ]
  })
