# filename: payment/services/capture_service.py
...
from payment.config.database import SessionLocal       # ✅ lowercase
...
async def capture_payment_internal(db: Session, payment: Payment, capture_amount: int):
    ...
    db.commit()
    db.refresh(payment)
    logger.info("Captured: %s - %s", payment.id, capture_amount)
...
async def auto_capture(payment_id: str, amount: int, merchant_id: str):
    db = SessionLocal()
    try:
        # ✅ Parenthesised chain — fixes the broken multi-line query
        payment = (
            db.query(Payment)
        ...
        logger.exception("Auto-capture failed: %s", e)
        db.rollback()
    finally:
        db.close()
