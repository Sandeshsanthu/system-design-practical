...
from models import MerchantWebhook, Notification, WebhookDelivery
from schemas import (
    DispatchResult,
...
async def notify(req: NotifyRequest, db: Annotated[AsyncSession, Depends(get_db)]):
...
async def stats(db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    active_wh = await db.scalar(
        select(func.count(MerchantWebhook.id)).where(MerchantWebhook.is_active.is_(True)))
...
async def get_notification(notification_id: str, db: Annotated[AsyncSession, Depends(get_db)]):
...
async def get_payment_notifications(payment_id: str, db: Annotated[AsyncSession, Depends(get_db)]):
...
async def register_webhook(req: WebhookRegisterRequest, db: Annotated[AsyncSession, Depends(get_db)]):
    wh = MerchantWebhook(
...
async def list_webhooks(merchant_id: str, db: Annotated[AsyncSession, Depends(get_db)]):
    result = await db.execute(
...
async def update_webhook(
        webhook_id: str, req: WebhookUpdateRequest, db: Annotated[AsyncSession, Depends(get_db)],
):
...
async def delete_webhook(webhook_id: str, db: Annotated[AsyncSession, Depends(get_db)]):
    wh = await db.get(MerchantWebhook, webhook_id)
    if not wh:
        raise HTTPException(404, "Webhook not found")
    await db.delete(wh)
...
async def rotate_webhook_secret(webhook_id: str, db: Annotated[AsyncSession, Depends(get_db)]):
    wh = await db.get(MerchantWebhook, webhook_id)
    if not wh:
        raise HTTPException(404, "Webhook not found")
