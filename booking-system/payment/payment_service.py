...
class PaymentService:
    ...
    def create_booking_with_payment(
        ...
        payment_started: Optional[float] = None
        ...
        with tracer.start_as_current_span("booking.create_with_payment") as span:
            try:
                ...
                payment_method_value = PaymentService._normalize_payment_method(
                    payment_method
                )
                payment_method_enum = PaymentService._get_payment_method_enum(
                ...
                with tracer.start_as_current_span("booking.validate_hold"):
                    ...
                    if not hold:
                        span.set_status(
                            Status(
                                StatusCode.ERROR,
                                "Seat hold is missing or expired",
                            )
                        )
                        raise HTTPException(
                            status_code=409,
                            detail="Seat hold is missing or expired",
                        )
                ...
                with tracer.start_as_current_span("booking.prepare_records"):
                    with acquire_lock(lock_key, timeout=10):
                        ...
                        if seat_status_value == "booked":
                            span.set_status(
                                Status(StatusCode.ERROR, "Seat is already booked")
                            )
                            raise HTTPException(
                                status_code=409,
                        ...
                        if seat_status_value != "available":
                            span.set_status(
                                Status(
                                    StatusCode.ERROR,
                                    f"Seat is not available: {seat_status_value}",
                                )
                            )
                            raise HTTPException(
                                status_code=409,
                        ...
                        event = (
                            db.query(Event)
                            .filter(Event.event_id == event_uuid)
                            .first()
                        )
                        if not event:
                ...
                with tracer.start_as_current_span("payment_gateway.process") as payment_span:
                    ...
                    payment_span.set_attribute(
                        "payment.amount",
                        PaymentService._to_float(seat.price),
                    )
                    ...
                    if not success:
                        payment_span.set_status(
                            Status(
                                StatusCode.ERROR,
                                gateway_response.get("error", "Payment failed"),
                            )
                        )
                ...
                with tracer.start_as_current_span("booking.finalize"):
                    with acquire_lock(lock_key, timeout=10):
                        ...
                        if not hold:
                            ...
                            raise HTTPException(
                                status_code=409,
                                detail="Seat hold expired during payment",
                            )
                        ...
                        if not seat or not payment or not booking:
                            span.set_status(
                                Status(
                                    StatusCode.ERROR,
                                    "Booking state could not be finalized",
                                )
                            )
                            raise HTTPException(
                                status_code=500,
                                detail="Booking state could not be finalized",
                            )
                        ...
                        if success:
                            ...
                            db.add(
                                PaymentService._create_audit_log(
                                    ...
                                    details=(
                                        "Payment successful: "
                                        f"{gateway_response.get('transaction_id')}"
                                    ),
                                )
                            )
                        ...
                        payment.failure_reason = gateway_response.get(
                            "error",
                            "Payment failed",
                        )
                        ...
                        db.add(
                            PaymentService._create_audit_log(
                                ...
                                new_status="failed",
                                details=(
                                    f"Payment failed: {gateway_response.get('error')}"
                                ),
                            )
                        )
                ...
                if payment_started is not None:
            ...
            try:
                ...
                if booking_status_value != "confirmed":
                    raise HTTPException(
                        status_code=400,
                        detail=(
                            f"Cannot refund booking in {booking_status_value} status"
                        ),
                    )
                ...
                with tracer.start_as_current_span("payment_gateway.refund") as refund_span:
                    ...
                    if not success:
                        refund_span.set_status(
                            Status(
                                StatusCode.ERROR,
                                refund_response.get("error", "Refund failed"),
                            )
                        )
                ...
                return {
                    ...
                    "seat_released": True,
                    "message": (
                        "Refund processed successfully and seat has been released"
                    ),
                }
